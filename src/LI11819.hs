...
-- | Posicao Inicial Gloss
posInicialY , posInicialX,tamanhoBloco :: Int
posInicialY = round $  (0.4 * fromIntegral(snd resolucaoEcra))
posInicialX = round $  - (0.4 * fromIntegral(fst resolucaoEcra))
tamanhoBloco = 25

-- | Resolucao do ecra
resolucaoEcra :: (Int,Int)
resolucaoEcra = (1400,800)
...
data Editor = Editor
    ...
    , tetrominoEditor :: Tetromino -- ^ Tipo do 'Tetromino' actual.
...
-- | Data type que me permite ver se o jogo está no inicio , a decorrer ou em pausa ou se alguem ganhou
data GameStatus = Start | InGame | InPause | Won deriving (Eq,Show)

-- | Estado Para O Gloss
data EstadoGloss = EstadoGloss
   { currentEstado :: Estado  -- ^ 'Estado' actual do Jogo
   , estadoInicial :: Estado  -- ^ 'Estado' Inicial do Jogo
   , mapaInt :: Int
   , imagemBIFundo :: (Picture,Picture)
   , imagemGloss   :: Picture -- ^ 'Picture' Mapa com as paredes indestrutiveis
   , gameState :: GameStatus  -- ^ 'GameStatus' Estado(Pausa, a decorrer , etc) do Jogo
   , botIsActive :: Maybe Int
   , teclas_press  :: [Key]   -- ^ Teclas Pressionadas pelos jogadores
   , timer :: Int
   } deriving (Show,Eq)
